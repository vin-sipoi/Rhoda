...
export async function POST(req: NextRequest) {
  try {
    ...
    console.log('Creating educator with data:', body);
    ...
    if (!response.ok) {
      ...
      return NextResponse.json({ 
        error: 'Failed to create educator profile', 
        details: errorText,
        status: response.status,
        url: `${STRAPI_URL}/api/educators`
      }, { status: response.status });
    }
...
export async function GET(req: NextRequest) {
  try {
    ...
    const response = await fetch(`${STRAPI_URL}/api/educators?populate=*`, {
      headers: {
        'Authorization': authorization,
        'Content-Type': 'application/json',
      },
    ...
    if (!response.ok) {
      return NextResponse.json({ error: 'Failed to fetch educators' }, { status: response.status });
    }
